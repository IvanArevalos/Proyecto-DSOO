...
{
    public partial class ProcesarCompraProveedoresHerramientas : Form
    {
        private List<ProductoStock> carrito = new List<ProductoStock>();
        public ProcesarCompraProveedoresHerramientas()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                using (MySqlConnection conexion = new MySqlConnection(textoconexion))
                {
                    ...
                    foreach (var item in carrito)
                    {
                        ...
                        string consultaExistente= @"SELECT COUNT(*) FROM herramientas WHERE Nombre=@Nombre AND Marca=@Marca AND Costo=@Costo AND Proveedor=@Proveedor AND Precio_bruto=@Precio_bruto";
                        ...
                        using (MySqlCommand comandoExistente= new MySqlCommand(consultaExistente, conexion))
                        {
                            ...
                            if (existe > 0)
                            {
                                string update = @"UPDATE herramientas SET Stock=Stock+@Stock WHERE Nombre=@Nombre AND Marca=@Marca AND Costo=@Costo AND Proveedor=@Proveedor AND Precio_bruto=@Precio_bruto";
                            ...
                            {
                                string consultainsert = @"INSERT INTO herramientas (Nombre,Marca,Stock,Costo,Proveedor,Precio_Bruto) VALUES (@Nombre,@Marca,@Stock,@Costo,@Proveedor,@Precio_bruto)";
