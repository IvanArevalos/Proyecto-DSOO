...
{
    public partial class ProcesarCompraProveedoresIndumentaria : Form
    {
        private List<ProductoStock> carrito = new List<ProductoStock>();
        public ProcesarCompraProveedoresIndumentaria()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                using (MySqlConnection conexion = new MySqlConnection(textoconexion))
                {
                    ...
                    foreach (var item in carrito)
                    {
                        ...
                        string consultaExistente= @"SELECT COUNT(*) FROM indumentaria WHERE Nombre=@Nombre AND Marca=@Marca AND Costo=@Costo AND Proveedor=@Proveedor AND Precio_bruto=@Precio_bruto";
                        ...
                        using (MySqlCommand comandoExistente= new MySqlCommand(consultaExistente, conexion))
                        {
                            ...
                            if (existe > 0)
                            {
                                string update = @"UPDATE indumentaria SET Stock=Stock+@Stock WHERE Nombre=@Nombre AND Marca=@Marca AND Costo=@Costo AND Proveedor=@Proveedor AND Precio_bruto=@Precio_bruto";
                            ...
                            {
                                string consultainsert = @"INSERT INTO indumentaria (Nombre,Marca,Stock,Costo,Proveedor,Precio_Bruto) VALUES (@Nombre,@Marca,@Stock,@Costo,@Proveedor,@Precio_bruto)";
